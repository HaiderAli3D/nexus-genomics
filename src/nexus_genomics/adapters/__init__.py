...
from nexus_genomics.adapters.felix_signatures import FelixSignaturesAdapter
...
ADAPTERS: Final[dict[str, Adapter]] = {
    ...
    "felix_signatures": FelixSignaturesAdapter(),
...
``codontransformer`` and ``seqscreen`` need no credentials. ``felix_signatures`` supplies a
narrower recorded-event contrast and needs one manual browser download. ``felix`` downloads an
